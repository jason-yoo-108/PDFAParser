...
class NameParser():
    def __init__(self, rnn_num_layers: int, format_rnn_hidden_size: int, dae_hidden_size: int, peak_prob: float = 1.):
        super().__init__()
        ...
        self.guide_fn = SequenceDenoisingModel(PRINTABLE, dae_hidden_size, NOISE, self.output_chars, encoder_num_layers=rnn_num_layers)
        self.guide_ln = SequenceDenoisingModel(PRINTABLE, dae_hidden_size, NOISE, self.output_chars, encoder_num_layers=rnn_num_layers)
        self.guide_suffix = ClassificationDenoisingModel(PRINTABLE, dae_hidden_size, NOISE, SUFFIX_LIST, encoder_num_layers=rnn_num_layers)
    ...
    def model(self, observations={"output": 0}):
        ...
        with torch.no_grad():
            ...
            if len(components[TITLE]) > 0:
                title, title_noise = sample_title_and_noise(components[TITLE])
                title_obs_probs = observation_probabilities(title, title_noise, self.peak_prob)
            if len(components[FIRST]) > 0:
                firstname, firstname_noise = sample_name_and_noise(components[FIRST], self.model_fn, ADDRESS['firstname'])
                firstname_obs_probs = observation_probabilities(firstname, firstname_noise, self.peak_prob)
            if len(components[MIDDLE]) > 0:
                middlenames, middlenames_noise = sample_multiple_name_and_noise(components[MIDDLE], self.model_fn, ADDRESS['middlename'])
            ...
            if len(components[LAST]) > 0:
                lastname, lastname_noise = sample_name_and_noise(components[LAST], self.model_ln, ADDRESS['lastname'])
                lastname_obs_probs = observation_probabilities(lastname, lastname_noise, self.peak_prob)
            if len(components[SUFFIX]) > 0:
                suffix, suffix_noise = sample_suffix_and_noise(components[SUFFIX])
    ...
    def guide(self, observations=None):
        ...
        if len(name_parse[TITLE]) > 0:
            ...
            title, title_noise = sample_title_and_noise(name_parse[TITLE], self.guide_title, encoder_output, encoder_hidden)
        if len(name_parse[FIRST]) > 0:
            ...
            firstname, firstname_noise = sample_name_and_noise(name_parse[FIRST], self.guide_fn, ADDRESS['firstname'], encoder_output, encoder_hidden)
        if len(name_parse[MIDDLE]) > 0:
            ...
            middlenames, middlenames_noise = sample_multiple_name_and_noise(name_parse[MIDDLE], self.guide_fn, ADDRESS['middlename'], encoder_outputs, encoder_hiddens)
        if len(name_parse[LAST]) > 0:
            ...
            lastname, lastname_noise = sample_name_and_noise(name_parse[LAST], self.guide_ln, ADDRESS['lastname'], encoder_output, encoder_hidden)
        if len(name_parse[SUFFIX]) > 0:
            ...
            suffix, suffix_noise = sample_suffix_and_noise(name_parse[SUFFIX], self.guide_suffix, encoder_output, encoder_hidden)
