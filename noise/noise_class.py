...
def sample_title_noise(title_format_length, title_noise_probs, rnn: ClassificationDenoisingModel = None,
    ...
    for i in range(title_format_length):
        ...
        noise_class = NOISE[
            pyro.sample(f"{ADDRESS['title']}_{ADDRESS['noise']}_{i}", dist.Categorical(char_noise_class_probs))]
...
def sample_suffix_noise(suffix_format_length, suffix_noise_probs, rnn: ClassificationDenoisingModel = None,
    ...
    for i in range(suffix_format_length):
        ...
        if suffix_length >= 3:
            char_noise_class_probs = forbid_noise_class(char_noise_class_probs, [NOISE_NONE, NOISE_ADD, NOISE_REPLACE])
        noise_class = NOISE[
            pyro.sample(f"{ADDRESS['suffix']}_{ADDRESS['noise']}_{i}", dist.Categorical(char_noise_class_probs))]
        noise_classes.append(noise_class)
